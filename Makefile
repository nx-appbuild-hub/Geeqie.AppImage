...
all: clean
	...
	apprepo --destination=$(PWD)/build appdir boilerplate geeqie geeqie-common
	echo "exec \$${APPDIR}/bin/geeqie \"\$${@}\"" >> $(PWD)/build/Boilerplate.AppDir/AppRun
	...
	export ARCH=x86_64 && bin/appimagetool.AppImage $(PWD)/build/Boilerplate.AppDir $(PWD)/Geeqie.AppImage
	chmod +x $(PWD)/Geeqie.AppImage
